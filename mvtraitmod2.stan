...
parameters {
  ...
  vector<lower=0>[T] sd_obsNC; //observation-level (residual) scale
  vector<lower=0>[T] sd_obsC; //observation-level (residual) scale
  cholesky_factor_corr[T] LR_obsNC; //observation-level (residual) corr
  cholesky_factor_corr[T] LR_obsC; //observation-level (residual) corr
...
transformed parameters{
  ...
  matrix[T,T] LP_obsNC = diag_pre_multiply(sd_obsNC, LR_obsNC);
  matrix[T,T] LP_obsC = diag_pre_multiply(sd_obsC, LR_obsC);
...
model {
  for(n in 1:N){
    //likelihood function
    if(charr_pres[n] == 0){
      traits[n] ~ multi_normal_cholesky(mu[n], LP_obsNC);
    }
    else{
      traits[n] ~ multi_normal_cholesky(mu[n], LP_obsC); 
    }
  }

  //priors
  to_vector(beta_defense) ~ normal(0,1);
  to_vector(beta_foraging) ~ normal(0,1);
  sd_obsNC ~ exponential(2);
  sd_obsC ~ exponential(2);
  sd_drain ~ exponential(2);
  sd_lake ~ exponential(2);
  LR_obsNC ~ lkj_corr_cholesky(2);
  LR_obsC ~ lkj_corr_cholesky(2);
...
generated quantities {
  //trait matrices
  matrix[T, T] RNC = LR_obsNC * LR_obsNC';
  matrix[T, T] PNC = LP_obsNC * LP_obsNC';
  matrix[T, T] RC = LR_obsC * LR_obsC';
  matrix[T, T] PC = LP_obsC * LP_obsC';
  vector[N] log_lik; //for computing LOO
